...
// Mock data — wire to API via TanStack Query when services are ready
const MOCK_KPIS = {
  todayPatients: 48,
  todayRevenue: 12_400,
  pendingConfirm: 7,
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {MOCK_ALERTS.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3">
          ...
          <div className="space-y-1">
            {MOCK_ALERTS.map((a) => (
              <p key={a.id} className="text-sm text-amber-800">{lang === 'ar' ? a.msgAr : a.msgEn}</p>
            ))}
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KpiCard
          ...
          value={formatCurrency(MOCK_KPIS.todayRevenue, 'EGP', lang === 'ar' ? 'ar-EG' : 'en-EG')}
          change={8.5}
          changeLabel={t('من أمس', 'vs yesterday')}
        ...
        <KpiCard
          ...
          titleAr="بانتظار التأكيد"
          value={MOCK_KPIS.pendingConfirm}
          change={-2}
        ...
        <KpiCard
          title="Avg Wait Time"
          titleAr="متوسط الانتظار"
          value={`${MOCK_KPIS.avgWait} ${t('د', 'min')}`}
          change={-5}
          changeLabel={t('من الأسبوع الماضي', 'vs last week')}
        ...
        <div className="lg:col-span-2">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-3">
              ...
              <span className="text-xs text-gray-400 font-normal">{t('الكل', 'See all')}</span>
            </CardHeader>
                ...
                <tbody>
                  {MOCK_APPOINTMENTS.map((appt) => (
                    <tr key={appt.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-gray-600 font-mono tabular-nums">
                      ...
                      <td className="px-5 py-3.5 text-gray-500 text-xs">{appt.source}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div>
          <div className="glass-primary rounded-2xl p-5 panel-floating">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold">{t('الجدول السريع', 'Quick Schedule')}</h3>
