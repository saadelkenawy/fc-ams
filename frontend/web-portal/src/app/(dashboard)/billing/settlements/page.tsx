...
import { Banknote, RefreshCw, ReceiptText, ChevronDown, ChevronRight, Building2, Stethoscope, Share2, Loader2 } from 'lucide-react';
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            {t('صافي ربح كل طرف: الوسيط، الطبيب، العيادة', 'Net profit per party: mediator, doctor, clinic')}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="rounded-xl border border-gray-100 dark:border-neutral-700 bg-white dark:bg-neutral-800 p-4">
            <div className="flex items-center gap-1.5 mb-1">
              ...
              <p className="text-xs text-gray-400">{t('إجمالي الإيرادات', 'Gross Revenue')}</p>
            </div>
          ...
          <div className="rounded-xl border border-orange-100 dark:border-orange-900/30 bg-orange-50 dark:bg-orange-900/10 p-4">
            ...
            {totalGross > 0 && <p className="text-xs text-orange-400 mt-0.5">{((totalMediator / totalGross) * 100).toFixed(1)}% {t('من الإجمالي', 'of gross')}</p>}
          </div>
          <div className="rounded-xl border border-blue-100 dark:border-blue-900/30 bg-blue-50 dark:bg-blue-900/10 p-4">
            ...
            {totalGross > 0 && <p className="text-xs text-blue-400 mt-0.5">{((totalDoctors / totalGross) * 100).toFixed(1)}% {t('من الإجمالي', 'of gross')}</p>}
          </div>
          <div className="rounded-xl border border-emerald-100 dark:border-emerald-900/30 bg-emerald-50 dark:bg-emerald-900/10 p-4">
            ...
            {totalGross > 0 && <p className="text-xs text-emerald-400 mt-0.5">{((totalClinic / totalGross) * 100).toFixed(1)}% {t('من الإجمالي', 'of gross')}</p>}
          </div>
        </div>
      )}
                  ...
                  <tr className="border-b border-gray-100 dark:border-neutral-700 bg-gray-50/60 dark:bg-neutral-800/60">
                    ...
                    <th className="text-end px-4 py-3 text-xs whitespace-nowrap">
                      <span className="flex items-center justify-end gap-1 text-blue-500 font-medium">
                        <Stethoscope className="w-3 h-3" />{t('حصة الطبيب', 'Doctor Share')}
                      </span>
                    </th>
                    <th className="text-end px-4 py-3 text-xs whitespace-nowrap">
                      <span className="flex items-center justify-end gap-1 text-emerald-500 font-medium">
                        <Building2 className="w-3 h-3" />{t('ربح العيادة', 'Clinic Profit')}
                      </span>
                    </th>
                    ...
                    return (
                      <>
                        ...
                        >
                          ...
                          <td className="px-4 py-3.5 text-end font-mono tabular-nums text-gray-700 dark:text-gray-200">{fmt(s.grossRevenue)}</td>
                          <td className="px-4 py-3.5 text-end font-mono tabular-nums text-orange-600 dark:text-orange-400">{fmt(s.totalSourceFees)}</td>
                          <td className="px-4 py-3.5 text-end font-mono tabular-nums text-gray-600 dark:text-gray-300">{fmt(netPool)}</td>
