...
export default function BillingPage() {
  ...
  const filtered = MOCK_TRANSACTIONS.filter((tx) => {
    const matchesQuery =
      (lang === 'ar' ? tx.patientAr : tx.patientEn).toLowerCase().includes(query.toLowerCase()) ||
      (lang === 'ar' ? tx.doctorAr : tx.doctorEn).toLowerCase().includes(query.toLowerCase());
    const matchesStatus = statusFilter === 'all' || tx.status === statusFilter;
    return matchesQuery && matchesStatus;
  });

  const totalRevenue = MOCK_TRANSACTIONS.reduce((s, t) => s + t.approved, 0);
  const totalDoctorShare = MOCK_TRANSACTIONS.reduce((s, t) => s + t.doctorShare, 0);
  const totalClinicShare = MOCK_TRANSACTIONS.reduce((s, t) => s + t.clinicShare, 0);
  ...
  return (
    <div className="space-y-5 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold font-display text-gray-900">{t('الفواتير والمالية', 'Billing & Finance')}</h2>
          <p className="text-sm text-gray-500 mt-0.5">{t('السجل المحاسبي غير القابل للتعديل', 'Immutable financial ledger')}</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard title="Total Revenue" titleAr="إجمالي الإيرادات"
          ...
          icon={<TrendingUp className="w-5 h-5" />} iconBg="bg-primary-50" lang={lang} featured />
        <KpiCard title="Doctor Share"  titleAr="حصة الأطباء"
          value={formatCurrency(totalDoctorShare, 'EGP', lang === 'ar' ? 'ar-EG' : 'en-US')}
          icon={<TrendingUp className="w-5 h-5" />} iconBg="bg-blue-50"    lang={lang} />
        <KpiCard title="Clinic Share"  titleAr="حصة العيادة"
          value={formatCurrency(totalClinicShare, 'EGP', lang === 'ar' ? 'ar-EG' : 'en-US')}
          icon={<CheckCircle className="w-5 h-5" />} iconBg="bg-emerald-50" lang={lang} />
        <KpiCard title="Pending"       titleAr="معلقة"
          value={pendingCount}
          icon={<Clock className="w-5 h-5" />}       iconBg="bg-amber-50"  lang={lang} />
      </div>

      {/* Tabs */}
      <div className="flex items-center gap-1 p-1 bg-white rounded-full shadow-1 border border-gray-100 w-fit">
        {TABS.map((tab) => (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-50 bg-gray-50/50">
                    <th className="text-start px-5 py-3 font-medium text-gray-500 text-xs">{t('التاريخ', 'Date')}</th>
                    ...
                    <th className="text-start px-5 py-3 font-medium text-gray-500 text-xs">{t('الإجراء', 'Procedure')}</th>
                    <th className="text-start px-5 py-3 font-medium text-gray-500 text-xs">{t('المصدر', 'Source')}</th>
                    <th className="text-end px-5 py-3 font-medium text-gray-500 text-xs">{t('الرسوم', 'Charge')}</th>
                    <th className="text-end px-5 py-3 font-medium text-gray-500 text-xs">{t('حصة الطبيب', 'Dr. Share')}</th>
                    <th className="text-end px-5 py-3 font-medium text-gray-500 text-xs">{t('حصة العيادة', 'Clinic')}</th>
                    <th className="text-start px-5 py-3 font-medium text-gray-500 text-xs">{t('الحالة', 'Status')}</th>
                    <th className="text-start px-5 py-3 font-medium text-gray-500 text-xs">{t('الدفع', 'Payment')}</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((tx) => {
                    ...
                    return (
                      <tr key={tx.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                        <td className="px-5 py-3.5 text-gray-500 text-xs">{formatDate(tx.date, lang === 'ar' ? 'ar-EG' : 'en-US')}</td>
                        <td className="px-5 py-3.5 font-medium text-gray-900">{lang === 'ar' ? tx.patientAr : tx.patientEn}</td>
                        <td className="px-5 py-3.5 text-gray-600">{lang === 'ar' ? tx.doctorAr : tx.doctorEn}</td>
                        <td className="px-5 py-3.5 text-gray-600">{lang === 'ar' ? tx.procedureAr : tx.procedureEn}</td>
                        <td className="px-5 py-3.5">
                          <Badge variant={['VEZ','EKF','DO'].includes(tx.source) ? 'info' : 'default'} className="text-xs">
                            {tx.source}
                          </Badge>
                        </td>
                        <td className="px-5 py-3.5 text-end font-mono tabular-nums font-medium text-gray-900">{tx.approved}</td>
                        <td className="px-5 py-3.5 text-end font-mono tabular-nums text-primary-700">{tx.doctorShare}</td>
                        <td className="px-5 py-3.5 text-end font-mono tabular-nums text-emerald-700">{tx.clinicShare}</td>
                        <td className="px-5 py-3.5">
                          <Badge variant={cfg.variant} dot>
                            {lang === 'ar' ? cfg.labelAr : cfg.labelEn}
                          </Badge>
                        </td>
                        <td className="px-5 py-3.5 text-gray-500 text-xs capitalize">{tx.payMethod.replace('_', ' ')}</td>
                      </tr>
                    );
                  })}
                  {filtered.length === 0 && (
                    <tr><td colSpan={10} className="px-5 py-12 text-center text-gray-400">{t('لا توجد نتائج', 'No results')}</td></tr>
                  )}
                </tbody>
              </table>
            </div>
      ...
      <CardContent className="p-0">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-50 bg-gray-50/50">
              <th className="text-start px-5 py-3 font-medium text-gray-500 text-xs">{t('الطبيب', 'Doctor')}</th>
              <th className="text-end px-5 py-3 font-medium text-gray-500 text-xs">{t('إجمالي الشهر', 'Gross (Month)')}</th>
              <th className="text-end px-5 py-3 font-medium text-gray-500 text-xs">{t('حصته', 'Dr. Share')}</th>
              <th className="text-end px-5 py-3 font-medium text-gray-500 text-xs">{t('مستحق', 'Pending Payout')}</th>
              <th className="text-start px-5 py-3 font-medium text-gray-500 text-xs">{t('الحالة', 'Status')}</th>
            ...
            {DOCTORS.map((d) => {
              ...
              return (
                <tr key={d.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                  <td className="px-5 py-3.5 font-medium text-gray-900">{lang === 'ar' ? d.nameAr : d.nameEn}</td>
                  <td className="px-5 py-3.5 text-end font-mono tabular-nums text-gray-700">{formatCurrency(d.gross, 'EGP', 'en-US')}</td>
                  <td className="px-5 py-3.5 text-end font-mono tabular-nums text-primary-700">{formatCurrency(d.share, 'EGP', 'en-US')}</td>
                  <td className="px-5 py-3.5 text-end font-mono tabular-nums font-semibold text-gray-900">
                    {d.pending > 0 ? formatCurrency(d.pending, 'EGP', 'en-US') : <span className="text-emerald-600 font-normal text-xs">{t('مسدد', 'Settled')}</span>}
                  </td>
