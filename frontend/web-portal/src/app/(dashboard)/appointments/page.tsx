  ...
  return (
    <div className="space-y-5 max-w-7xl mx-auto">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold font-display text-gray-900">{t('المواعيد', 'Appointments')}</h2>
        </div>
        <Button size="sm">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        <div className="flex items-center gap-2">
          <button onClick={() => shiftDate(-1)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500">
          ...
          <input
            ...
            className="h-9 rounded-lg border border-gray-200 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary-600"
          />
          <button onClick={() => shiftDate(1)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500">
        ...
        <div className="flex items-center gap-1 p-1 bg-white rounded-full shadow-1 border border-gray-100 overflow-x-auto">
          {STATUS_TABS.map((tab) => (
            <button
              key={tab.status}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-50 bg-gray-50/50">
                <th className="text-start px-5 py-3 font-medium text-gray-500 text-xs">{t('الوقت', 'Time')}</th>
                ...
                <th className="text-start px-5 py-3 font-medium text-gray-500 text-xs">{t('الرسوم', 'Charge')}</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((a) => (
                <tr key={a.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors cursor-pointer">
                  <td className="px-5 py-3.5 font-mono text-gray-600 text-xs">{formatTime(a.time)}</td>
                  <td className="px-5 py-3.5 font-medium text-gray-900">{lang === 'ar' ? a.patientAr : a.patientEn}</td>
                  <td className="px-5 py-3.5 text-gray-600">{lang === 'ar' ? a.doctorAr : a.doctorEn}</td>
                  <td className="px-5 py-3.5 text-gray-600">{lang === 'ar' ? a.specialtyAr : a.specialtyEn}</td>
                  <td className="px-5 py-3.5"><AppointmentStatusBadge status={a.status} lang={lang} /></td>
                  <td className="px-5 py-3.5 font-mono text-gray-700 tabular-nums">{a.charge} {t('ج', 'EGP')}</td>
                </tr>
              ))}
              {filtered.length === 0 && (
                <tr><td colSpan={6} className="px-5 py-12 text-center text-gray-400">{t('لا توجد مواعيد', 'No appointments found')}</td></tr>
              )}
            </tbody>
          </table>
        </CardContent>
