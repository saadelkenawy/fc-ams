...
export async function buildApp(): Promise<ReturnType<typeof Fastify>> {
  ...
  await app.register(swagger, {
    openapi: {
      openapi: '3.0.0',
      info: { title: 'Fadl Clinic — Patient Service', version: '1.0.0', description: 'Patient management API' },
      tags: [{ name: 'patients', description: 'Patient CRUD and search' }],
      components: {
        securitySchemes: {
  ...
  app.get('/health', { logLevel: 'silent' }, async () => ({ status: 'ok', service: 'patient-service' }));
