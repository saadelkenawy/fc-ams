...
export async function previewCancel(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  ...
  if (!preview) {
    reply.status(404).send({ success: false, error: { code: 'NOT_FOUND', message: 'Queue entry not found or not cancellable' } });
    return;
  }
...
export async function getPosition(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  ...
  if (!entry) {
    reply.status(404).send({ success: false, error: { code: 'NOT_FOUND', message: 'Queue entry not found' } });
    return;
  }
  reply.send({ success: true, data: entry });
}
